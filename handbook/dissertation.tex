...
\caption{The Tree Modification Process}
\label{fig:processes}
\end{figure}
...
\section{Implementing the language}
 In this section I will outline how I implemented the language, and detail the data structures and algorithms used. 

 
\subsection{Outline of General Process}

For each feature that I introduced to CodeTouch there were three main processes that it had to fit into, with changes in one often causing problems in the other. They were:

\begin{enumerate}
\item Ensuring that only the correct buttons are displayed to the user
\item Ensuring that the code would output on the screen correctly
\item Ensuring that the code ran properly
\end{enumerate}

Because of this I had to work very carefully, making sure to thoroughly test the application whenever developing additional features. 

In order to aid this process I created a set of unit tests that I ran every time I changed anything, a set which grew as the project went on. This made sure that I didn't break the functionality of CodeTouch without realising as I was developing. Android for Java allows you to programatically perform button presses which meant that I could build up tests that would simulate any sequence of possible button presses, which was very useful for testing. 

\begin{quote}
\begin{lstlisting}[caption={Example Unit Test},label={lst:testCode},language=Java]
public void blankFunctionTestOneParam(){
...
I learnt early on that it was best to try and keep these 3 processes as separate as possible. Initially, I used the button presses to determine both the modification that should be performed on the tree and the buttons that should be displayed. This made things difficult as often different button presses result in the program being in the same state.
...
\subsection{CodeTouch vs TouchDevelop}
...
TouchDevelop also is not a syntax-mistake free environment, making it less nurturing than CodeTouch, although it does have a strong real time error reporting system.
